			     ...
			     u8 *data, uint64_t *dsize)
{
	...
	*dsize = cpu_to_be64(*dsize);
	...
	rc = opal_secvar_get(ptr_to_opal(key), ksize, ptr_to_opal(data),
			     ptr_to_opal(dsize));
				  ...
				  uint64_t keybufsize)
{
	...
	*keylen = cpu_to_be64(*keylen);
	...
	rc = opal_secvar_get_next(ptr_to_opal(key), ptr_to_opal(keylen),
				  keybufsize);
			     ...
			     uint64_t dsize)
{
	...
	rc = opal_secvar_enqueue_update(ptr_to_opal(key), ksize, ptr_to_opal(data),
					dsize);
